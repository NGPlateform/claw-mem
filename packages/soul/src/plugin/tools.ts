// Agent tools for the COC soul layer: backup, restore, guardian, recovery, carrier.
//
// Migrated from packages/claw-mem/src/tools/soul-tools.ts when the plugin
// surface moved out of the claw-mem super-skill. Takes the three soul
// managers directly instead of the umbrella CliServices object.
...
export function registerSoulTools(api: PluginApi, deps: SoulToolDeps): void {
  const { backupManager, recoveryManager, carrierManager, logger } = deps
