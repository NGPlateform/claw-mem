...
const SOUL_REGISTRY_ABI = [
  ...
  // Events (required for _extractEventArg; ethers sets log.fragment from ABI)
  ...
  "event ResurrectionInitiated(bytes32 indexed requestId, bytes32 indexed agentId, bytes32 carrierId, uint8 trigger)",
...
export class SoulClient {
  ...
  async listCarriers(opts: {
    ...
    for (const ev of regEvents) {
      const id = (ev as unknown as { args: [string] }).args?.[0]
      if (id) seen.set(id)
    }
    if (!includeInactive) {
      for (const ev of deregEvents) {
        const id = (ev as unknown as { args: [string] }).args?.[0]
    ...
    const infos = await Promise.all(ids.map((id) => this.getCarrier(id).catch(() => null)))
    return infos.filter((x): x is CarrierInfo => x !== null && (includeInactive || x.active))
  }
