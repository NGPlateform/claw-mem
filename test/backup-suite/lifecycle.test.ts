...
import { buildDoctorReport, resolveRestorePlan, runInitFlow } from "../../src/services/lifecycle.ts"
import { writeBackupState, writeLatestRecoveryPackage } from "../../src/services/local-state.ts"
import type { BackupReceipt } from "../../src/services/backup-types.ts"
...
describe("lifecycle", () => {
  it("runInitFlow registers a soul and reports generated recovery metadata", async () => {
    ...
    assert.equal(result.registrationTxHash, "0xregister")
    assert.equal(result.backupReceipt.status, "completed")
    assert.ok(result.recoveryPackagePath, "recoveryPackagePath should not be null")
    assert.match(result.recoveryPackagePath, /latest-recovery\.json$/)
  })
  ...
  it("resolveRestorePlan requires password when local recovery package says so", async () => {
    ...
    assert.equal(plan.manifestCid, "bafylatest")
    assert.equal(plan.isPassword, true)
    assert.equal(plan.source, "latest-local")
  })
  ...
  it("buildDoctorReport marks registered soul without backups as registered_no_backup", async () => {
    ...
    assert.equal(report.state, "registered_no_backup")
    assert.equal(report.actions.some((action) => action.id === "first_backup"), true)
  })
})
